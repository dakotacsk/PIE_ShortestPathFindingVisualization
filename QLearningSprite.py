...
class QLearningSprite:
    def __init__(self, start_position, cell_size, rows, cols, alpha=0.1, gamma=0.99, epsilon=0.6, max_steps=5000):
        ...
        self.recent_positions = []  # Memory of recent positions to detect oscillations
    ...
    def take_step(self, grid):
        ...
        while not valid_move:
            ...
            if 0 <= new_row < self.rows and 0 <= new_col < self.cols:
                ...
                # Detect and prevent oscillation (e.g., moving back and forth between two states)
                if len(self.recent_positions) > 1 and new_position == self.recent_positions[-2]:
                    reward = -50  # Higher penalty for oscillating
                ...
                # Allow movement over red blocks but with penalties handled elsewhere
                if grid.maze[new_row][new_col] == RED:
                    # Apply heavy penalty for moving onto a red block
                    reward = -500
                    self.q_table[state][action] += self.alpha * (reward - self.q_table[state][action])  # Update Q-table
                    print(f"Moved onto a red block at {new_position}. Heavy penalty applied: {reward}")
                else:
                    reward = -1  # Default penalty for a valid move
                    reward += grid.get_reward(new_position)

                # Track position history to detect oscillations
                self.recent_positions.append(self.position[:])
                if len(self.recent_positions) > 10:  # Keep track of the last 10 moves
                ...
                # Apply penalty for invalid move (attempting to go out of bounds)
                reward = -15
                self.q_table[state][action] += self.alpha * (reward - self.q_table[state][action])  # Update Q-table
